...
class FabricaEnergia:
    ...
    def crear_fuente(datos_configuracion):
        ...
        if tipo_energia == "solar":
            ...
            raise ValueError("Tipo de energía no reconocida.")
...
class InterfazRed:
    def mostrar_tablero(self, lista_fuentes):
        ...
        print(ancho_interno)
        print(f"| {'ID':^10} | {'TIPO':^12} | {'ESTADO/RECURSOS':^15} | {'PRODUCCIÓN':^14} |")
        print(ancho_interno)
...
class ControladorRed:
    ...
    def ejecutar_sistema(self):
        ...
        clima_ingresado = self.vista_sistema.solicitar_entrada("Ingrese clima (Soleafo/Nublado): ")
        viento_ingresado = self.vista_sistema.solicitar_entrada("Ingrese velocidad del viento: ")
        diesel_ingesado = self.vista_sistema.solicitar_entrada("Litros de diesel para respaldo: ")
        ...
        for fuente in self.lista_fuentes_energia:
            if isinstance(fuente, PanelSolar):
                ...
                fuente.generar(diesel_ingesado)
        
        self.vista_sistema.mostrar_mensaje_sistema("Simulación completada. Actualizando tablero...")
        self.vista_sistema.mostrar_mensaje_sistema(self.lista_fuentes_energia)

if __name__ == "__main__":
    aplicacion_red = ControladorRed()
    aplicacion_red.ejecutar_sistema()