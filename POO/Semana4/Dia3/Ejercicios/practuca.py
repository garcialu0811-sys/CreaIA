...
class EntidadBase(ABC):
    def __init__(self, nombre_id, energia_inicial):
        ...
        if energia_inicial < 0:
            raise EnergiaInvalidaError("La energia inicial no puede ser un valor negatativo.")
...
class Sintetizador(EntidadBase):
    def alimentarse(self, cantidad_energia=10):
        ...
        print(f"Sintetizador {self.nucleo_vital.id_entidad} absorbió luz ambiental. Energia: {self.energia_actual}")
...
def iniciar_ciclo_vital(lista_de_entidades):
    print("\n--- INICIANDO PROCESO DE ALIMENTACION DE LA COLONIA ---")
