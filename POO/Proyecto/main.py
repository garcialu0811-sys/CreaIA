...
def seleccionar_pokemon(mensaje_instruccion):
    while True:
        ...
        try:
            ...
            print("Error en la selección: " + str(error))

# Funcion que controla la logica de cada turno
def ejecutar_turno(jugador, oponente, controlador=False):
    ...
    if controlador:
        ...
        print("La computadora elige la opcion: " + str(opcion_accion))
    else:
        ...
        while True:
            try:
                ...
                if opcion_accion == 1 or opcion_accion == 2 or opcion_accion == 3:
                    break
    ...
    # Ejecutamos los métodos definidos en la clase base y los polimorficos
    if opcion_accion == 1:
...
# Funcion principal que orquesta toda la simulacion
def iniciar_simulador():
    ...
    while luchador_uno.esta_vivo() and luchador_dos.esta_vivo():
        ...
        es_el_turno_del_primero = not es_el_turno_del_primero

    # Declaración del ganador una vez que alguien se queda sin vida
    print("\n" + "X"*30)
    if luchador_uno.esta_vivo():
        print("¡EL GANADOR ES " + luchador_uno.nombre + "!")
    else:
        print("¡EL GANADOR ES " + luchador_dos.nombre + "!")
    print("X"*30)
